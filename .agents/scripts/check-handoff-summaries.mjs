...
function runSelfTest() {
  ...
  const leaseManager = scriptPath("lease-manager.mjs");
  const zone = ".agents/scripts/lease-manager.mjs";
  mkdirSync(dir, { recursive: true });
  try {
    const acquire = spawnSync(process.execPath, [leaseManager, "acquire", "--state", state, "--owner", "handoff-test", "--project", "delano-multi-agent-execution", "--task", "T-006", "--zone", zone], { cwd: repoRoot, encoding: "utf8" });
...
function scriptPath(name) {
  const agentsPath = path.join(repoRoot, ".agents", "scripts", name);
