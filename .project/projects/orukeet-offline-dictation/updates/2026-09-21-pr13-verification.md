...
## Third review repair batch — 533c7ed
...
- Undo feedback expires with its offer. Restore cancellation propagates while validation errors remain sanitized. Probe audio cleanup covers failed binding, timeout and activity destruction; cache cleanup runs after failed download/validation. An injected runtime-download failure leaves no temporary cache file. Mirror preparation explicitly rejects an existing regular file.
- Verification: 478 app tests and 23 offline-ASR tests pass, debug assembly and release Kotlin compilation pass, and the standalone probe builds. Five optimized Python evidence tests pass. Benchmark/instrumentation APK build evidence remains from the preceding batch; device execution and public qualification remain pending.
- Atomic dictionary fallback is retained: it copies the previous document before ordinary replacement, and startup recovery reads that backup if the main file is absent or unreadable. The injected overwrite-rejection regression and existing backup recovery tests remain green.
