...
| `DictationRecordingBar` | Uses live `audioLevelFlow`, but repeats one current value through a fixed profile; pause/cancel controls are 30 or 34 dp. | Refactor to rolling independent samples and 48 dp controls; do not wire the current component unchanged. |
